...
export function calculateSale(draft: SaleDraft) {
  if (!Number.isInteger(draft.quantity) || draft.quantity <= 0) throw new Error('Quantity must be a positive whole number');
  ...
  const subtotalMinor = draft.unitPriceMinor * draft.quantity;
  ...
  const costMinor = draft.product.standardCostMinor * draft.quantity;
